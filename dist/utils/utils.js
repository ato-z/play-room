 ...
 * ```
*/
var date = function (dateString, _date) {
    var reg = /[y|m|d|h|i|s]/ig;
    _date ? (_date instanceof Date || (_date = new Date(_date))) : (_date = new Date());
    dateString = dateString.replace(reg, function (val) {
        val = val.toUpperCase();
        if (val === 'Y') {
            return _date.getFullYear().toString();
        }
        if (val === 'M') {
            return (0, exports.fillZero)(_date.getMonth() + 1);
        }
        if (val === 'D') {
            return (0, exports.fillZero)(_date.getDate());
        }
        if (val === 'H') {
            return (0, exports.fillZero)(_date.getHours());
        }
        if (val === 'I') {
            return (0, exports.fillZero)(_date.getMinutes());
        }
        if (val === 'S') {
            return (0, exports.fillZero)(_date.getSeconds());
        }
        return '';
    });
    return dateString;
 ...
 * ```
 */
var touchPath = function (path) {
    var diffPath = [];
    var decodePath = path.split('\/');
    var popHandle = function () {
        try {
            var currentPath = decodePath.join('\/');
            if (decodePath.length) {
                (0, fs_1.accessSync)(currentPath);
            }
        }
        catch (err) {
            var diff = decodePath.pop();
            diffPath.push(diff);
            popHandle();
        }
    };
